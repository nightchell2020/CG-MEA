...
from .train_core import train_multistep, train_multistep_IL
...
def learning_rate_search(
    ...
    max_log_lr = -6.0 + np.log10(config["minibatch"] * config.get("ddp_siz3090 e", 1) / 32)
...
def train_script_IL(
    ...
    # generate the trainers
    optimizer = get_optimizer(new_model, config)
    scheduler = get_lr_scheduler(
        optimizer,
        config["lr_scheduler_type"],
        iterations=config["iterations"],
    ...
    # local variable for training loop
    best_val_acc = 0
    best_model_state = deepcopy(new_model.state_dict())
    i_step = 0
    ...
    if main_process:
        ...
        if config["use_wandb"]:
            ...
            # track gradients and weights statistics if needed
            if config.get("watch_model", False):
                wandb.watch(new_model, log="all", log_freq=history_interval, log_graph=True)
    ...
    while i_step < config["iterations"]:
        ...
        # train during 'history_interval' steps
        loss, train_acc = train_multistep_IL(
            pre_model=pre_model,
            new_model=new_model,
            loader=train_loader,
            preprocess=preprocess_train,
