...
class VGG1D(nn.Module):
    def __init__(self, model: str, in_channels: int, out_dims: int, seq_length: int, use_age: str,
    ...
    def _make_conv_stage(self, conv_filter, cfg, base_channels):
        ...
        for k in range(cfg['layers']):
            ...
            if self.batch_norm:
                conv_layers += [nn.Conv1d(in_channels=self.current_channels,
    ...
    def compute_feature_embedding(self, x, age, target_from_last: int = 0):
        ...
        if self.use_age == 'conv':
            age = age.reshape((N, 1, 1)).repeat((1, 1, L))
