        ...
        for i in range(self.fc_stages - 1):
            ...
            if hasattr(self.class_heads, linear_name) and isinstance(getattr(self.class_heads, linear_name), nn.Linear):
                ...
                if self.activation == 'tanh':
                    nn.init.trunc_normal_(getattr(self.class_heads, linear_name).weight, std=math.sqrt(1 / fan_in))
                else:
                    nn.init.trunc_normal_(getattr(self.class_heads, linear_name).weight, std=math.sqrt(2.0 / fan_in))
