...
def get_constant_with_decay_scheduler(
    optimizer: Optimizer, iterations: int, last_epoch: int = -1
):
    return lr_scheduler.StepLR(
        optimizer, step_size=round(iterations * 0.8), gamma=0.1, last_epoch=last_epoch
    )


def get_constant_with_twice_decay_scheduler(
    optimizer: Optimizer, iterations: int, last_epoch: int = -1
):
...
def get_transformer_style_scheduler(
    optimizer: Optimizer, warmup_steps: int, last_epoch: int = -1
):
    ...
    return lr_scheduler.LambdaLR(
        optimizer, transformer_style_lambda, last_epoch=last_epoch
    )
...
def get_linear_decay_with_warmup(
    optimizer: Optimizer, warmup_steps: int, iterations: int, last_epoch: int = -1
):
    def linear_decay_with_warmup(step: int):
        if step <= warmup_steps:
            return step / max(1.0, float(warmup_steps))
        return max(
            0.0, (iterations - step) / max(1.0, float(iterations - warmup_steps))
        )
...
def get_lr_scheduler(
    ...
    if scheduler_type == "constant_with_decay":
        return get_constant_with_decay_scheduler(
        ...
        return get_transformer_style_scheduler(
            optimizer=optimizer, warmup_steps=warmup_steps, last_epoch=last_epoch
        )
