...
def get_constant_with_decay_scheduler(optimizer: Optimizer, iterations: int, last_epoch: int = -1):
    return lr_scheduler.StepLR(optimizer,
                               step_size=round(iterations * 0.8),
                               gamma=0.1,
...
def get_transformer_style_scheduler(optimizer: Optimizer, warmup_steps: int, last_epoch: int = -1):
    def transformer_style_lambda(step: int):
        return min(math.sqrt(warmup_steps) / max(1.0, math.sqrt(step)),
                   step / max(1.0, float(warmup_steps)))

    return lr_scheduler.LambdaLR(optimizer,
                                 transformer_style_lambda,
                                 last_epoch=last_epoch)


def get_cosine_decay_with_warmup(optimizer: Optimizer, warmup_steps: int,
                                 iterations: int, cycles: float = 0.5, last_epoch: int = -1):
...
def get_linear_with_warmup(optimizer: Optimizer, warmup_steps: int, iterations: int, last_epoch: int = -1):
    def linear_with_warmup(step: int):
        if step <= warmup_steps:
            return step / max(1.0, float(warmup_steps))
        return max(0.0, (iterations - step) / max(1.0, float(iterations - warmup_steps)))

    return lr_scheduler.LambdaLR(optimizer, linear_with_warmup, last_epoch)


def get_lr_scheduler(optimizer: Optimizer, scheduler_type: str,
                     iterations: int, warmup_steps: int, last_epoch: int = -1):

    if scheduler_type == 'constant_with_decay':
        return get_constant_with_decay_scheduler(optimizer=optimizer,
                                                 iterations=iterations,
                                                 last_epoch=last_epoch)
    elif scheduler_type == 'transformer_style':
        return get_transformer_style_scheduler(optimizer=optimizer,
                                               warmup_steps=warmup_steps,
                                               last_epoch=last_epoch)
    elif scheduler_type == 'cosine_decay_with_warmup_half':
        return get_cosine_decay_with_warmup(optimizer=optimizer,
                                            warmup_steps=warmup_steps,
                                            iterations=iterations,
                                            cycles=0.5,
                                            last_epoch=last_epoch)
